...
SELECT EMPNO, ENAME,
	CASE WHEgN DEPTNO = (SELECT DEPTNO FROM DEPT WHERE LOC = 'NEW YORK')
		THEN '본사'
		ELSE  '분점'
	END AS "소속"
FROM EMP
...
ALTER TABLE EMP_ALTER 
	MODIFY EMPNO NUMBER(5); -- 크기를 크게 하는 건 가능
	
ALTER TABLE EMP_ALTER
	MODIFY TEL VARCHAR(18); -- 크기를 줄였지만 영향 받는 해당 컬럼에 대한 데이터가 영향을 받지 않으면 가능
...
INSERT INTO TABLE_NOTNULL VALUES ('SYW2024', 'AASSSS', NULL);

SELECT *
FROM TABLE_NOTNULL;

ARTER TABLE TABLE_NOTNULL
	MODIFY TEL NOT NULL;

-- UPDATE는 DML이며, 열의 데이터를 수정
UPDATE TABLE_NOTNULL
	SET TEL = '010-1111-2222'
	WHERE LOGIN_ID = 'SYW2024';
...
INSERT INTO TABLE_UNIQUE VALUES (NULL, 'ayj1234', '010-1234-5678');
